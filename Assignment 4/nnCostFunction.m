...
h = h2;
for i = 1 : num_labels
    yv = (y == i);  % yk means the k-th label
    hv = h(:,i);      % hk means the k-th output
    J = J+(-1 / m)*(yv' * log(hv) + (1 - yv)' * log(1 - hv));
end
Thtemp1 = Theta1(:,2:end);
Thtemp2 = Theta2(:,2:end);
theta_temp = [Thtemp1(:); Thtemp2(:)];
